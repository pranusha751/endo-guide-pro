  ...
  .handler(async ({ data }): Promise<AuthResponse> => {
    try {
      ...
      if (error) {
        ...
        return { user: null, error: (error as any).message || JSON.stringify(error) || "Database insert failed" };
      }
      ...
      return { user: null, error: msg === "{}" ? "An unexpected server error occurred." : msg };
    }
  });
