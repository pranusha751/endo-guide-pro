...
const searchSchema = z.object({
  ...
  files: z.enum(FILE_SYSTEMS as unknown as [string, ...string[]]).optional().default("ProTaper Gold"),
});


export const Route = createFileRoute("/workflow/summary")({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <PageHeader 
          title="Case Summary" 
          subtitle="Review and finalize your endodontic record" 
        />
      </div>
      ...
      <div className="bg-primary/10 border border-primary/20 rounded-2xl p-5 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-primary-foreground/70">Treatment plan successfully generated</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <Card className="rounded-2xl border-border bg-card shadow-card">
          <CardHeader className="pb-2 border-b border-border/50">
            <CardTitle className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Clinical Details</CardTitle>
          </CardHeader>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Button variant="outline" className="rounded-2xl h-auto py-4 flex flex-col gap-1 border-border bg-card shadow-card">
        ...
        <Button className="w-full rounded-2xl py-6 text-lg font-bold shadow-soft gap-2">
...
function SummaryRow({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="flex justify-between items-center py-1">
      ...
      <span className={`text-sm font-bold ${color ? `text-${color}-foreground bg-${color}/20 px-2 py-0.5 rounded-lg` : "text-foreground"}`}>
        {value}
      </span>
    </div>
  );
