    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">
          {filter ? `${filter} Cases` : "Recent Cases"}
        </h3>
        {cases.length > 0 && (
        ...
        {displayedCases.length === 0 ? (
          <div className="text-center py-8 bg-card rounded-2xl border border-dashed border-border/60 shadow-sm">
            <p className="text-sm text-muted-foreground mb-4">
              {filter ? `No ${filter.toLowerCase()} cases found.` : "No cases yet."}
            </p>
            {!filter && (
              <Link
                to="/workflow"
          ...
          displayedCases.map((c) => (
            ...
            >
              <article className="rounded-2xl bg-card border border-border p-4 shadow-card hover:border-primary/30 transition-colors">
...
function Stat({
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full rounded-2xl p-4 text-center transition-all duration-200 active:scale-95 ${
        isActive
          ? "bg-primary text-primary-foreground shadow-lg scale-105 z-10 border-primary"
          : "bg-card border border-border text-card-foreground shadow-card hover:border-primary/50"
      }`}
    >
      <Icon
