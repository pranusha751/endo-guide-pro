...
import { signInWithPassword, signInWithGoogle, sendMagicLink } from "@/lib/auth-stub";
...
function RouteComponent() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setSuccessMsg(null);
    setLoading(true);
    // Supabase-ready: swap signInWithPassword in src/lib/auth-stub.ts
  ...
  const handleGoogleLogin = async () => {
    ...
    // As requested, clicking Google Sign-in triggers the email verification simulation
      ...
      <Card className="w-full max-w-sm rounded-2xl shadow-sm border-0 sm:border">
        ...
        <CardContent>
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            {successMsg && (
              <p className="rounded-md bg-mint/20 px-3 py-2 text-sm text-mint-foreground">
                {successMsg}
              </p>
            )}
