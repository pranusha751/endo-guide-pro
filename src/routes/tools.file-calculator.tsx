          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-xl bg-mint/30 p-3">
                <div className="text-[10px] font-medium uppercase text-mint-foreground/70">Speed</div>
                <div className="text-sm font-semibold text-mint-foreground">{protocol.rpm}</div>
              </div>
              <div className="rounded-xl bg-mint/30 p-3">
                <div className="text-[10px] font-medium uppercase text-mint-foreground/70">Torque</div>
...
function Section({ title, items, icon: Icon, color }: { title: string; items: string[]; icon: any; color: "mint" | "peach" }) {
  return (
    <Card className="rounded-2xl border-border bg-card overflow-hidden">
      <div className={`px-4 py-2 bg-${color}/20 border-b border-${color}/20 flex items-center gap-2`}>
        <Icon className={`w-4 h-4 text-${color}-foreground`} />
        <span className={`text-xs font-bold uppercase tracking-wider text-${color}-foreground`}>{title}</span>
      </div>
      <CardContent className="p-4">
        <ul className="space-y-3">
          {items.map((item, i) => (
            <li key={i} className="flex gap-3 items-start">
              <span className={`w-5 h-5 rounded-full bg-${color} text-${color}-foreground text-[10px] font-bold flex items-center justify-center flex-shrink-0 mt-0.5`}>
                {i + 1}
              </span>
